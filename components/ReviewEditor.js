...
export default class AnnotationEditor extends Component {
	constructor(props) {
		...
		this.state = {
			...
			windowWidth: 0,
			refs: {},
			src_entity_annotations: {},
			dst_entity_annotations: {}
			}
	...
	getDestinationEntityAnnotationPositions(id) {
		var self = this
		...
		axios.get('/api/entity_annotation_get', {
			params: params
		})
		.then(function (response) {
			
			const entity_annotation = response.data
			var start = entity_annotation.start
			var end = entity_annotation.end	

			self.setState(prevState => {
				let dst_entity_annotations = Object.assign({}, prevState.dst_entity_annotations);
				dst_entity_annotations[id] = [start, end];
				return { dst_entity_annotations };
			  })
	...
	getEntityAnnotationPositions(id) {
		var self = this
		...
		.then(function (response) {
			...
			self.setState(prevState => {
				let src_entity_annotations = Object.assign({}, prevState.src_entity_annotations);
				src_entity_annotations[id] = new Array(start, end);
				return { src_entity_annotations };
			  })
	...
	refreshFromDB() {
		...
		.then(function (response) {
			...
			self.setState( {
				relation_annotations: relations,
		...
		.then(function () {
			// always executed
			self.state.relation_annotations.forEach(function (item, index){
				
				self.getEntityAnnotationPositions(item.srcId)
				self.getDestinationEntityAnnotationPositions(item.dstId)
			})
	...
	render() {
		...
		const relation_types = {1:"Upregulates", 2:"Downregulates", 3:"Diagnostic", 4:"Predisposing", 5:"Predictive", 6:"Prognostic"}
		
		

		const rows = this.state.relations.map(r => 
		<tr key={r.id}>
			<td>{relation_types[r.relationTypeId]}</td>
			<td>{this.state.src_entity_annotations[r.srcId]}</td>
			<td>{this.state.dst_entity_annotations[r.dstId]}</td>
			<td><RelationAnnotation 
			key={r.srcId+'_'+r.dstId} 
			src={getEntity(r.srcId)} 
			dst={getEntity(r.dstId)} 
			relation={r} 
			accept={this.addRelationAnnotationToDB}
			reject={evt => rejectRelation(r)}
			windowWidth={this.state.windowWidth} /></td>
		</tr>)
		

		var contents = <Table striped bordered hover>
			<thead>
				<tr>					
					<th>Relation Type</th>
					<th>Source Entity</th>
		...
		return (
			...
			<div>{docMetadata}{tree}{relations}{contents}</div>
