...
export default class SenEditor extends Component {
	constructor(props) {
		super(props)
		this.state = {
		...
		this.refreshSpansFromDB = this.refreshSpansFromDB.bind(this);
		this.refreshRelationAnnotationsFromDB = this.refreshRelationAnnotationsFromDB.bind(this);
	...
	deleteEntityAnnotationFromDB(attribs) {
		var self = this
		axios.get('/api/update_data/delete_entity_annotation', {
			params: {
				id: attribs.entityAnnotationId
			}
		})
		.then(function (response) {
			self.refreshSentenceFromDB()
			self.refreshSpansFromDB()
			self.refreshRelationAnnotationsFromDB()
		})
	...
	addEntityAnnotationToDB(entityId) {
		...
		var self = this
		axios.get('/api/update_data/add_entity_annotation', {
			params: {
	...
	refreshSpansFromDB() {
		...
		var fetchURL = '/api/get_data/get_entity_annotations'
		...
		.then(function (response) {
			...
			entity_annotations.forEach(function (item, index) {
				var span = []
				span.push(item['start'])
				span.push(item['end']-item['start'])
				span.push('EntityAnnotation')

				var dict = {'description':'entity annotation',
							'entityAnnotationId': item['id'],
							'key': 'EntityAnnotation_' + String(item['start']) + '_' + String(item['end']-item['start']),
	...
	refreshRelationAnnotationsFromDB(){
		var self = this

		var fetchURL = '/api/get_data/get_relation_annotations'
		var params = {sentence_id: this.props.sentence_id}
		...
		.then(function (response) {
			const relation_annotations = response.data
			var relations = []
			

			relation_annotations.forEach(function (item, index) {
				var relation = {'dst_id': item['entity_two_id'], 'id':item['id'], 'relation_type':'Diagnostic', 'relation_type_id':1, 'src_id': item['entity_one_id']}
	...
	componentDidMount() {
		...
		this.refreshSpansFromDB()
		this.refreshRelationAnnotationsFromDB()
	}
	...
	render() {
		...
		*/
		...
		if (this.state.relations){
			...
			console.log(relations_with_candidate)
			const rejectRelation = r => r.isCandidate ? this.clearCandidateRelation() : this.deleteRelationAnnotationFromDB(r)
		
			var relations = relations_with_candidate.map( r => <RelationAnnotation 
			key={r.src_id+'_'+r.dst_id} 
			src={getEntity(r.src_id)} 
			dst={getEntity(r.dst_id)} 
			relation={r} 
